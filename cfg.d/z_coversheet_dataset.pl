...
sub get_system_field_info
{
	...
	( 
		...
		{ name => "frontfile_hash", type => "text", text_index => 0 },
		{ name => "backfile_hash",  type => "text", text_index => 0 },
	);
}
...
sub search_by_doc
{
        ...
        return 0 unless( $format eq "application/pdf" || $mime_type eq "application/pdf" || $filename =~ /\.pdf$/i );

        # ignore thumbnails e.g. http://.../8381/1.haspreviewThumbnailVersion/jacqueline-lane.pdf
        foreach my $rel ( @{$relations || []} )
        {
                return 0 if( $rel =~ /^is\w+ThumbnailVersionOf$/ );
        }

        # ignore volatile documents
        return 0 if $doc->has_relation( undef, "isVolatileVersionOf" );

        my $session = $doc->get_session;

        # search for a coversheet that can be applied to this document
        return $self->search_by_eprint( $session, $doc->get_eprint );
}
...
sub erase_page
{
	...
	$self->set_value( $fieldname . "_hash", undef);
...
# run a search based on the search string provided by the coversheet's apply_to field
sub applies_to_eprint
{
	...
	my $session = $self->{session};
        ...
        $searchexp->from_string_raw( $self->value( "apply_to" ) );
	$searchexp->add_field(
		fields => [
			$ds->field( 'eprintid' ),
		],
	...
	my $eprints = $searchexp->perform_search;
	if( $eprints->count == 1 && $eprint->id eq $eprints->item( 0 )->id )
	{ 
		return 1;
	}
	return 0;
...
# rwf1v07:25/10/2019  Have deprecated the following as it appears to apply a more simple search rather than utilise EPrints actual search meaning more complex searchers such as date ranges don't work
#
# Checks that this CS applies to $eprint (tests done in-memory). The original code (above) was testing via database lookup.
sub applies_to_eprint_DEPR2
{
	my( $self, $eprint ) = @_;
...
sub calculate_coverdata_hash
{
	my( $class, $session, $eprint, $doc ) = @_;
